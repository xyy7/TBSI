...
def build_tbsi_track(cfg, training=True):
    ...
    if cfg.MODEL.PRETRAIN_FILE and ("TBSITrack" not in cfg.MODEL.PRETRAIN_FILE) and training:
        ...
        pretrained = "/home/xyy/TBSI/pretrained_models/TBSITrack_SOT_Pretrained.pth.tar"
        print("Load pretrained model from: " + pretrained)
    else:
        pretrained = ""

    if cfg.MODEL.BACKBONE.TYPE == "vit_base_patch16_224_tbsi":
        backbone = vit_base_patch16_224_tbsi(pretrained, drop_path_rate=cfg.TRAIN.DROP_PATH_RATE, tbsi_loc=cfg.MODEL.BACKBONE.TBSI_LOC, tbsi_drop_path=cfg.TRAIN.TBSI_DROP_PATH)
    else:
        raise NotImplementedError
    ...
    model = TBSITrack(backbone, box_head, aux_loss=False, head_type=cfg.MODEL.HEAD.TYPE)
